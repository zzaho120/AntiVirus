...
public class EquippableSO 
{
    ...
    {
        string[] files = Directory.GetFiles("Assets/Resources/Choi/Datas/Equippables");
        ...
        using (FileStream fs = new FileStream(Application.dataPath + equippableCSVPath, FileMode.Open, FileAccess.Read))
        {
            using (StreamReader sr = new StreamReader(fs))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    AssetDatabase.CreateAsset(equippable, $"Assets//Resources/Choi/Datas/Equippables/{equippable.name}.asset");
                    equippableNum++;
                }
                sr.Close();
            }
